...
from .common import UserRequest
from .timers.timers_command import timers_parser
from .timers.timers_processor import process_timers_request
from .top.top_processor import process_top_request
from .top.top_command import top_command, TopCommandArgs
from .help_processor import (
    help_command,
    process_help_request,
)
from .presets.presets_processor import (
    process_presets,
)
from .presets.presets_command import (
    presets_command,
    presets_parser
)
...
from common.command_parser.exception import TooManyArgumentsError
...
_logger = create_logger(__name__, config.LOG_LEVEL)
...
async def process_message(
    message: UserRequest, bot_name: str, api: Slacker, db_service: str
) -> None:
    """
    ...
    :param db_service: URL for DB operations
    """
    ...
    try:
        ...
        if not parse_result:
            ...
            text_to_answer = await process_top_request(top_command_args, db_service, message.user)
            ...
            text_to_answer = await process_presets(parse_result.sub_parser_result, message.user, db_service)
